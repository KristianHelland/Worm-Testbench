...
restart = false;
    ...
    if(play){
        ...
        if ((wormHead.x >= gridSize) || (wormHead.y >= gridSize) || (wormHead.x < 0) || (wormHead.y < 0)) {
            restartButton.addEventListener("click", restartButton.onclick(location.reload()))
        ...
        wormHead = {x: 5,y:Math.floor(gridSize/2)}
        //halen til marken begynner midt i rutenettet og to fliser fra høyre
        wormTail = {x: 2,y:Math.floor(gridSize/2)}
        return
        }
